...
このページは、Action が「どうやって堅固なシステムとして実装されているか」を説明するための資料です。
ハッカソンの審査や技術説明で、単に「堅牢です」と言うのではなく、どの設計判断で壊れにくくしているかを具体的に伝える用途を想定しています。
...
> Action の堅牢性は、後から fallback を足して守ったものではありません。  
> 複数の入力経路を内部では canonical event に寄せ、重い処理は Pub/Sub 経由の非同期 pipeline に逃がし、`act-api`, `act-adk-worker`, `organize` を責務で分離することで、最初から壊れにくい構造にしています。

少し長く言うなら次です。

> このプロダクトは、資料 upload、large import、Discord live message という異なる入力を扱います。  
> そこで、入口ごとに別ロジックを増やすのではなく、内部では `input.received` のような canonical event に寄せました。  
> また、重い知識化は同期 API に載せず、Pub/Sub 経由の Organize pipeline に切り出しています。  
> さらに、認証は `act-api`、推論は `act-adk-worker`、知識更新は `organize` に分離し、正本データと raw data の保存先も分けています。  
> この境界設計によって、応答性、一貫性、拡張性を同時に確保しています。
